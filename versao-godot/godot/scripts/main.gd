...
func _sync_local_match_setup_state(start_button: Button = null) -> void:
	...
	if start_button != null:
		start_button.disabled = active_count < 2
...
func _show_quit_menu() -> void:
	...
	# Add spacing and logo
	_add_spacer(_scaled_classic_value(110.0))
	_add_logo()
	_add_spacer(_scaled_classic_value(110.0))

	# Panel containing "Are you sure?" and Yes/No buttons
	var classic_panel_size := Vector2(718.0, 180.0)
	var panel := PanelContainer.new()
	panel.add_theme_stylebox_override("panel", GroundfireTheme.classic_panel_style())
	panel.custom_minimum_size = _scaled_menu_size(classic_panel_size)
	panel.size_flags_horizontal = Control.SIZE_SHRINK_CENTER
	...
	# "Are you sure?" label using ClassicLabel
	var label := ClassicLabel.new()
	label.text = "Are you sure?"
	label.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	label.size_flags_horizontal = Control.SIZE_SHRINK_CENTER
	var label_font_size := int(round(float(30) * _menu_scale()))
	GroundfireTheme.apply_label(label, label_font_size, GroundfireTheme.COLOR_TEXT, true)
	menu.add_child(label)

	# Yes and No buttons
	var menu_buttons: Array[Button] = []
	menu_buttons.append(_add_classic_button_to(menu, "Yes", func() -> void: get_tree().quit()))
	menu_buttons.append(_add_classic_button_to(menu, "No", _show_main_menu))
...
func _show_options(back_callback: Callable = Callable()) -> void:
	...
	var reset_controls := _add_button_to(controls_section, "Reset To Defaults", func() -> void:
		ControlSettings.reset_defaults()
		_show_options(back_callback)
	)
...
func _begin_key_capture(action_name: String) -> void:
	...
	if _capture_prompt != null:
		_capture_prompt.text = _classic_control_capture_prompt(action_name)
...
func _classic_control_capture_prompt(action_name: String) -> String:
	return "Press Button for '%s'" % ControlSettings.display_name(action_name)
