...
func _check_main_menu_and_options() -> void:
	...
	# Test Quit Menu transitions
	main.call("_on_quit")
	await process_frame
	await process_frame
	assert(_has_label(main, "Are you sure?"))
	assert(_has_button(main, "Yes"))
	assert(_has_button(main, "No"))
	var no_button := _find_control_with_text(main, "Button", "No") as Button
	assert(no_button != null)
	...
	assert(_has_label(main, "3 players ready  Human 0  Computer 3"))
	roster = main.call("_local_match_roster_snapshot")
	assert(roster.size() == 3)
	assert(str(Dictionary(roster[0]).get("kind", "")) == "computer")
	first_slot.select(0)
	main.call("_on_local_match_setup_row_changed", 0)
	assert(not setup_start.disabled)
	...
	assert(not _has_label(main, "Select a control to rebind."))
	assert(_has_button(main, "Set Controls"))
	assert(_has_button(main, "Reset To Defaults"))
	...
	var reset_to_defaults := _find_control_with_text(main, "Button", "Reset To Defaults") as Button
	...
	main.call("_begin_key_capture", "gf_fire")
	assert(_has_label(main, "Press Button for 'Fire Weapon'"))
	main.call("_begin_gamepad_capture", "gf_weapon_next")
	assert(_has_label(main, "Press Button for 'Change Weapon Up'"))
	main.call("_cancel_input_capture")
	assert(not _has_label(main, "Press Button for 'Change Weapon Up'"))
