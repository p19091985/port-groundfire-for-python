...
func _run() -> void:
	...
	for action_name in classic_keyboard_defaults:
		assert(int(ControlSettings.DEFAULT_BINDINGS[action_name]) == int(classic_keyboard_defaults[action_name]))
	var classic_action_display_names := {
		"gf_fire": "Fire Weapon",
	...
	var classic_rebind_actions: Array[String] = [
		"gf_fire",
		"gf_weapon_next",
		"gf_weapon_prev",
		"gf_jump",
	...
	assert(ControlSettings.DEFAULT_BINDINGS.has("gf_pause"))
	assert(not ControlSettings.action_names().has("gf_pause"))
	assert(ControlSettings.action_names() == classic_rebind_actions)
	var gf_fire_events := InputMap.action_get_events("gf_fire")
	InputMap.action_erase_events("gf_fire")
	assert(ControlSettings.key_label("gf_fire") == "<Undefined>")
	assert(ControlSettings.gamepad_label("gf_fire") == "<Undefined>")
	for event in gf_fire_events:
		InputMap.action_add_event("gf_fire", event)
	var classic_gamepad_buttons := {
		"gf_fire": 0,
		"gf_weapon_next": 2,
		"gf_weapon_prev": 1,
		"gf_jump": 3,
	...
	for action_name in classic_gamepad_buttons:
		assert(int(ControlSettings.DEFAULT_GAMEPAD_BUTTONS[action_name]) == int(classic_gamepad_buttons[action_name]))
	assert(ControlSettings._gamepad_button_label(0) == "Joy Button 1")
	assert(ControlSettings._gamepad_button_label(2) == "Joy Button 3")
	assert(ControlSettings._gamepad_button_label(1) == "Joy Button 2")
	assert(ControlSettings._gamepad_button_label(6) == "Joy Button 7")
	assert(ControlSettings._gamepad_button_label(7) == "Joy Button 8")
	var classic_gamepad_axes := {
	...
	for action_name in classic_gamepad_axes:
		var expected_axis: Dictionary = classic_gamepad_axes[action_name]
		var actual_axis: Dictionary = ControlSettings.DEFAULT_GAMEPAD_AXES[action_name]
		assert(int(actual_axis["axis"]) == int(expected_axis["axis"]))
		assert(abs(float(actual_axis["value"]) - float(expected_axis["value"])) < 0.01)
	assert(ControlSettings._gamepad_axis_label(JOY_AXIS_LEFT_X, -1.0) == "Joystick/Pad Left")
	assert(ControlSettings._gamepad_axis_label(JOY_AXIS_LEFT_X, 1.0) == "Joystick/Pad Right")
	assert(ControlSettings._gamepad_axis_label(JOY_AXIS_LEFT_Y, 1.0) == "Joystick/Pad Up")
	assert(ControlSettings._gamepad_axis_label(JOY_AXIS_LEFT_Y, -1.0) == "Joystick/Pad Down")
	assert(not ControlSettings.DEFAULT_GAMEPAD_AXES.has("gf_move_left"))
	assert(not ControlSettings.DEFAULT_GAMEPAD_AXES.has("gf_move_right"))
	assert(ControlSettings.CLASSIC_LINKED_ACTIONS["gf_weapon_next"] == "gf_weapon_prev")
	assert(ControlSettings.CLASSIC_LINKED_ACTIONS["gf_move_left"] == "gf_move_right")
	assert(ControlSettings.CLASSIC_LINKED_ACTIONS["gf_aim_left"] == "gf_aim_right")
	assert(ControlSettings.CLASSIC_LINKED_ACTIONS["gf_power_up"] == "gf_power_down")
	ControlSettings.reset_gamepad_defaults()
	ControlSettings.save_gamepad_axis_binding("gf_aim_left", JOY_AXIS_LEFT_X, -1.0)
	assert(ControlSettings.gamepad_label("gf_aim_left") == "Joystick/Pad Left")
	assert(ControlSettings.gamepad_label("gf_aim_right") == "Joystick/Pad Right")
	ControlSettings.save_gamepad_axis_binding("gf_power_up", JOY_AXIS_LEFT_Y, 1.0)
	assert(ControlSettings.gamepad_label("gf_power_up") == "Joystick/Pad Up")
	assert(ControlSettings.gamepad_label("gf_power_down") == "Joystick/Pad Down")
	ControlSettings.save_gamepad_axis_binding("gf_aim_left", JOY_AXIS_LEFT_X, -1.0)
	ControlSettings.save_gamepad_button_binding("gf_aim_left", 5)
	assert(ControlSettings.gamepad_label("gf_aim_left") == "Joy Button 6")
	assert(ControlSettings.gamepad_label("gf_aim_right") == "<Undefined>")
	ControlSettings.clear_gamepad_binding("gf_aim_left")
	assert(ControlSettings.gamepad_label("gf_aim_left") == "<Undefined>")
	ControlSettings.reset_gamepad_defaults()
	...
	var score_cancel_event := InputEventAction.new()
	score_cancel_event.action = "ui_cancel"
	score_cancel_event.pressed = true
	local_match.call("_unhandled_input", score_cancel_event)
	assert(str(local_match.get("_phase")) == "score")
	assert(local_match.get_node("ScoreOverlay").visible)
	var score_accept_event := InputEventAction.new()
	score_accept_event.action = "ui_accept"
	score_accept_event.pressed = true
	local_match.call("_unhandled_input", score_accept_event)
	...
	var human_winner_host := MainMenuHost.new()
	root.add_child(human_winner_host)
	var human_winner_match = LocalMatchScene.instantiate()
	human_winner_match.setup({"total_rounds": 1})
	human_winner_host.add_child(human_winner_match)
	await process_frame
	await process_frame
	human_winner_match.call("_open_winner_overlay")
	human_winner_match.call("_update_modal_activation", 2.0)
	assert(abs(float(human_winner_match.get("_winner_continue_delay"))) < 0.01)
	var winner_cancel_event := InputEventAction.new()
	winner_cancel_event.action = "ui_cancel"
	winner_cancel_event.pressed = true
	human_winner_match.call("_unhandled_input", winner_cancel_event)
	assert(int(human_winner_host.main_menu_calls) == 0)
	assert(str(human_winner_match.get("_phase")) == "winner")
	var winner_accept_event := InputEventAction.new()
	winner_accept_event.action = "ui_accept"
	winner_accept_event.pressed = true
	human_winner_match.call("_unhandled_input", winner_accept_event)
	assert(int(human_winner_host.main_menu_calls) == 0)
	assert(str(human_winner_match.get("_phase")) == "winner")
	var winner_fire_event := InputEventAction.new()
	winner_fire_event.action = "gf_fire"
	winner_fire_event.pressed = true
	human_winner_match.call("_unhandled_input", winner_fire_event)
	assert(int(human_winner_host.main_menu_calls) == 1)
	await _free_node(human_winner_host)
	...
	assert(first_catalog_item.text == "Machine Gun")
	assert(not str(first_catalog_item.text).contains("Stock"))
	assert(not str(first_catalog_item.text).contains("Pack"))
	assert(not str(first_catalog_item.text).contains("Damage"))
	assert(not str(first_catalog_item.text).contains("Blast"))
	assert(int(first_catalog_row.get_meta("classic_shop_stock", -1)) == 50)
	assert(int(first_catalog_row.get_meta("classic_shop_pack", -1)) == 50)
	var jump_jet_catalog_row = shop_weapon_list.get_child(2) as HBoxContainer
	var jump_jet_catalog_item = jump_jet_catalog_row.get_child(1) as Label
	assert(jump_jet_catalog_item.text == "Jump Jet")
	assert(not str(jump_jet_catalog_item.text).contains("Current"))
	assert(str(jump_jet_catalog_row.get_meta("classic_shop_effect", "")) == "+100% reserve")
	assert(str(jump_jet_catalog_row.get_meta("classic_shop_current", "")) == "100% reserve")
	...
	assert(mirv_catalog_item.text == "Mirvs")
	...
	assert(missile_catalog_item.text == "Missiles")
	...
	assert(nuke_catalog_item.text == "Nukes")
	...
	assert(first_locked_item.text == "Rolling Mines")
	assert(first_locked_row.get_child_count() == 2)
	assert(int(first_locked_row.get_meta("classic_shop_position", -1)) == 5)
	assert(bool(first_locked_row.get_meta("classic_shop_selected", false)))
	assert(first_locked_cost.get_theme_color("font_color") == GroundfireTheme.COLOR_TEXT)
	assert(first_locked_item.get_theme_color("font_color") == GroundfireTheme.COLOR_TEXT)
	...
	shop.refresh({
		...
		"inventory": [
			{"name": "Shell", "cost": 0, "ammo": -1, "damage": 40, "blast": 48},
			{"name": "Machine Gun", "cost": 50, "ammo": 50, "stock": 50, "shop_pack": 50, "damage": 2, "blast": 0},
			{"name": "MIRV", "cost": 50, "ammo": 1, "stock": 1, "shop_pack": 1, "damage": WeaponInventory.MIRV_DAMAGE, "blast": 34},
			{"name": "Missile", "cost": 50, "ammo": 0, "stock": 5, "shop_pack": 5, "damage": 40, "blast": 48},
			{"name": "Nuke", "cost": 50, "ammo": 1, "stock": 1, "shop_pack": 1, "damage": 90, "blast": 96},
		],
		"shop_items": [
			{"name": "Jump Jet", "cost": 50, "effect": "+100% reserve", "current": "200% reserve"},
		],
		"message": "Selected limited stock check",
		"selected_position": 3,
	})
	await process_frame
	shop_weapon_list = shop.get("_weapon_list") as VBoxContainer
	var selected_missile_row = shop_weapon_list.get_child(4) as HBoxContainer
	assert(selected_missile_row.get_child_count() == 4)
	var selected_missile_item = selected_missile_row.get_child(1) as Label
	assert(selected_missile_item.text == "Missiles")
	assert(not str(selected_missile_item.text).contains("x5"))
	var selected_missile_stock = selected_missile_row.get_child(2) as Label
	assert(selected_missile_stock.text == "x5")
	assert(int(selected_missile_row.get_meta("classic_shop_stock", -1)) == 5)
	assert((selected_missile_row.get_child(3) as Button).text == "Buy")
	shop.refresh({
		"title": "Round Won",
		"round": 2,
		"total_rounds": 5,
		...
		"inventory": [
			{"name": "Shell", "cost": 0, "ammo": -1, "damage": 40, "blast": 48},
			{"name": "Machine Gun", "cost": 50, "ammo": 50, "stock": 125, "shop_pack": 50, "damage": 2, "blast": 0},
			{"name": "MIRV", "cost": 50, "ammo": 1, "stock": 1, "shop_pack": 1, "damage": WeaponInventory.MIRV_DAMAGE, "blast": 34},
			{"name": "Missile", "cost": 50, "ammo": 0, "stock": 5, "shop_pack": 5, "damage": 40, "blast": 48},
			{"name": "Nuke", "cost": 50, "ammo": 1, "stock": 1, "shop_pack": 1, "damage": 90, "blast": 96},
		],
		"shop_items": [
			{"name": "Jump Jet", "cost": 50, "effect": "+100% reserve", "current": "250% reserve"},
		],
		"message": "Selected Machine Gun bar check",
		"selected_position": 0,
	})
	await process_frame
	shop_weapon_list = shop.get("_weapon_list") as VBoxContainer
	var selected_machine_gun_row = shop_weapon_list.get_child(1) as HBoxContainer
	assert(selected_machine_gun_row.get_child_count() == 4)
	var selected_machine_gun_item = selected_machine_gun_row.get_child(1) as Label
	assert(selected_machine_gun_item.text == "Machine Gun")
	var selected_machine_gun_bars = selected_machine_gun_row.get_child(2) as VBoxContainer
	assert(str(selected_machine_gun_bars.get_meta("classic_shop_indicator_kind", "")) == "bars")
	assert(abs(float(selected_machine_gun_bars.get_meta("classic_shop_bar_value", 0.0)) - 2.5) < 0.01)
	assert(selected_machine_gun_bars.get_child_count() == 3)
	assert(abs(float((selected_machine_gun_bars.get_child(0) as ColorRect).get_meta("classic_shop_bar_fraction", 0.0)) - 1.0) < 0.01)
	assert(abs(float((selected_machine_gun_bars.get_child(2) as ColorRect).get_meta("classic_shop_bar_fraction", 0.0)) - 0.5) < 0.01)
	assert((selected_machine_gun_row.get_child(3) as Button).text == "Buy")
	shop.refresh({
		"title": "Round Won",
		"round": 2,
		"total_rounds": 5,
		...
		"message": "Selected Jump Jet bar check",
		"selected_position": 1,
	})
	await process_frame
	shop_weapon_list = shop.get("_weapon_list") as VBoxContainer
	var selected_jump_jet_row = shop_weapon_list.get_child(2) as HBoxContainer
	assert(selected_jump_jet_row.get_child_count() == 4)
	var selected_jump_jet_item = selected_jump_jet_row.get_child(1) as Label
	assert(selected_jump_jet_item.text == "Jump Jet")
	var selected_jump_jet_bars = selected_jump_jet_row.get_child(2) as VBoxContainer
	assert(str(selected_jump_jet_bars.get_meta("classic_shop_indicator_kind", "")) == "bars")
	assert(abs(float(selected_jump_jet_bars.get_meta("classic_shop_bar_value", 0.0)) - 2.5) < 0.01)
	assert(selected_jump_jet_bars.get_child_count() == 3)
	assert(abs(float((selected_jump_jet_bars.get_child(2) as ColorRect).get_meta("classic_shop_bar_fraction", 0.0)) - 0.5) < 0.01)
	assert((selected_jump_jet_row.get_child(3) as Button).text == "Buy")
	shop.refresh({
		"title": "Round Won",
		"round": 2,
		"total_rounds": 5,
	...
	assert(int((shop.get("_continue_button") as Button).get_meta("classic_shop_position", -1)) == 10)
	assert(bool((shop.get("_continue_button") as Button).get_meta("classic_shop_selected", false)))
	...
	local_match.set("_shop_input_delay", -0.01)
	local_match.call("_set_current_shop_select_pos", 0)
	local_match.set("_credits", 50)
	var cursor_shop_inventory: RefCounted = local_match.get("_inventory")
	var cursor_machine_gun_before = int(cursor_shop_inventory.stock_for(WeaponInventory.MACHINE_GUN))
	local_match.call("_handle_classic_shop_command", "up")
	assert(int(local_match.call("_current_shop_select_pos")) == 10)
	assert(int(local_match.get("_credits")) == 50)
	assert(int(cursor_shop_inventory.stock_for(WeaponInventory.MACHINE_GUN)) == cursor_machine_gun_before)
	assert(abs(float(local_match.get("_shop_input_delay")) - 0.2) < 0.01)
	var cursor_shop_overlay := local_match.get("_shop_overlay") as Control
	var cursor_shop_state: Dictionary = cursor_shop_overlay.get("_state")
	...
	local_match.call("_prepare_shop_pass")
	local_match.set("_phase", "shop")
	local_match.set("_shop_input_delay", 0.2)
	...
	assert(abs(shield_tank.fuel - TankState.TANK_FULL_FUEL) < 0.01)
	assert(abs(shield_tank.fuel_reserve - TankState.TANK_FULL_FUEL) < 0.01)
	assert(int(shield_tank.damage_after_shield(40)) == 40)
	...
	assert(abs(float(player_tank.fuel) - TankState.TANK_FULL_FUEL) < 0.01)
