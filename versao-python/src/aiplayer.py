...
from .commandintents import PlayerIntentFrame
from .common import PI, deg_cos, deg_sin, sqr
    ...
    def update(self, time: float = 0.0):
        ...
        self.publish_intents(
            PlayerIntentFrame.from_iterable(
                self._commands,
                source="ai",
                simulation_time=self._game.get_time(),
            )
        )
                ...
                if not self._ignore_shot:
                    if self._target_tank:
                        ...
                        if self._last_shot and (
                            ...
                            or (curr_x_dist > 0.0 and prev_x_dist > 0.0 and curr_x_dist > prev_x_dist)
                        ):
    ...
    def compute_action(self):
        if not self._target_tank:
            ...
            if self._target_tank._state == Tank.TANK_ALIVE:
                if sqr(self._target_tank._x - self._target_last_x_pos) + sqr(self._target_tank._y - self._target_last_y_pos) > 4.0:
                    self._target_tank = None
                else:
                    ready_to_fire = True
                    ...
                    if ready_to_fire and self._tank.ready_to_fire():
                        if self._aim_directly:
                            if sqr(self._target_tank._x - self._target_last_x_pos) + sqr(self._target_tank._y - self._target_last_y_pos) > 0.04:
                                ready_to_fire = False
                                self.guess_aim()
