...
from groundfire_net.observability import EventLogFile, GroundfireEventLogger
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        choices=("text", "json"),
        default="text",
        help="Network event log format used with --log-network-events or --log-file.",
    )
    parser.add_argument(
        "--log-file",
        default="",
        help="Append client network event logs to this file. JSON format writes JSON Lines.",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.log_file:
        with EventLogFile(args.log_file) as stream:
            return _run_client(args, stream=stream)
    return _run_client(args, stream=None)


def _run_client(args: argparse.Namespace, *, stream: TextIO | None) -> int:
    client = ClientApp(
        network_backend="udp",
        secure_server_public_key_path=args.server_public_key,
        event_logger=_build_event_logger(args, stream=stream),
    )
...
def _build_event_logger(args: argparse.Namespace, *, stream: TextIO | None):
    if stream is None and not args.log_network_events:
        return None
    return GroundfireEventLogger(
        "groundfire-client",
        stream=stream,
        json_lines=args.log_format == "json",
    )
