...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        choices=("text", "json"),
        default="text",
        help="Event log format used with --log-events or --log-file.",
    )
    parser.add_argument(
        "--log-file",
        default="",
        help="Append server event logs to this file. JSON format writes JSON Lines.",
    )
    return parser
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.log_file:
        with EventLogFile(args.log_file) as stream:
            return _run_server(args, stream=stream)
    return _run_server(args, stream=None)


def _run_server(args: argparse.Namespace, *, stream: TextIO | None) -> int:
...
def _build_event_logger(args: argparse.Namespace, *, stream: TextIO | None):
    if stream is None and not args.log_events:
        return None
