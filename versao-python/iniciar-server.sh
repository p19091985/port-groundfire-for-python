...
# shellcheck source=versao-python/scripts/launcher_common.sh
...
launch_pygame_menu() {
    ...
    PYTHONPATH="$PYTHONPATH_PREFIX${PYTHONPATH:+:$PYTHONPATH}" "$python_bin" -m src.groundfire.app.dedicated_server_menu "$SCRIPT_PATH" "$PROJECT_DIR" "$LOG_FILE" 2>&1 | tee -a "$LOG_FILE"
    return "${PIPESTATUS[0]}"
}
...
wait_for_server() {
    ...
    PYTHONPATH="$PYTHONPATH_PREFIX${PYTHONPATH:+:$PYTHONPATH}" "$python_bin" - "$host" "$port" "$timeout_seconds" <<'PY'
import socket
import sys
import time
...
start_server() {
    ...
    PYTHONPATH="$PYTHONPATH_PREFIX${PYTHONPATH:+:$PYTHONPATH}" "${cmd[@]}" >> "$LOG_FILE" 2>&1 &
    SERVER_PID=$!
    ...
    if ((show_client)); then
        ...
        PYTHONPATH="$PYTHONPATH_PREFIX${PYTHONPATH:+:$PYTHONPATH}" "${client_cmd[@]}" >> "$client_log" 2>&1 &
        VISIBLE_CLIENT_PID=$!
