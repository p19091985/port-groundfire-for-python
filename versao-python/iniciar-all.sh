...
launch_ttk_menu() {
    ...
    PYTHONPATH="$PYTHONPATH_PREFIX${PYTHONPATH:+:$PYTHONPATH}" "$python_bin" - "$SCRIPT_PATH" "$PROJECT_DIR" "$LOG_FILE" 2>&1 <<'PY' | tee -a "$LOG_FILE"
import os
import re
import shlex
import subprocess
...
start_automatic_game() {
    ...
    local server_script="$SCRIPT_DIR/iniciar-server.sh"
    local clients_script="$SCRIPT_DIR/iniciar-clientes.sh"
