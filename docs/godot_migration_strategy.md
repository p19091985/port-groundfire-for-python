...
- Latest local validation run for this slice: `CI=1 .venv/bin/python scripts/run_quality_checks.py` passed on 2026-06-07 with `359 passed, 12 subtests passed`. The latest `test_port_fidelity.py` expansion added 4 missing tests for Machine Gun turn flow edge cases: `test_machine_gun_fire_starts_looped_audio_and_unselect_stops_it`, `test_machine_gun_pre_shot_cancellation_stops_audio_before_firing`, `test_machine_gun_lethal_hit_stops_firing_and_audio`, and `test_ai_tactical_hold_fires_machine_gun_continuously`. This locks in the classic weapon-cycle audio loop, pre-shot cancellation, death cancellation, and AI continuous hold path before Godot-side tuning. All 359 tests pass. `scripts/run_quality_checks.py` (`compileall`, `unittest`, `ruff`, `mypy`) all pass.
