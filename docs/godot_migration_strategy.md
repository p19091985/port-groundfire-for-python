...
- **Status de conclusão**: The local Godot migration/release slice is currently green across the strongest local gates run on 2026-06-07. Do not call the whole public migration `100% complete` until hosted production, GitHub release publishing, signing secrets, and account/session policy are proven against real infrastructure.
- **Auditoria Codex em 2026-06-07 apos reparo**:
  - `CI=1 .venv/bin/python scripts/run_quality_checks.py` passed `compileall`, `unittest`, `ruff`, and `mypy`.
  - `.venv/bin/python -m pytest -q tests/test_groundfire_net_module.py` passed with `18 passed`, including active WebSocket frame tests through a fake UDP backend.
  - `scripts/validate_godot_release.sh` passed with `135 passed`.
  ...
  - `scripts/validate_godot_release.sh --browser-qa --package` passed as the combined local release gate, covering migration contract validation, fidelity validation, exported-browser runtime QA, exported-browser visual QA, release packaging, and checksum verification in one run.
...
- **Hosting & Deployment**: The intended public Web target is documented as `https://play.groundfire.net/`, but the repository still does not prove that this host is live or serving the current package. `Dockerfile`, `Caddyfile`, and `docker-compose.yml` now provide deployment scaffolding for Caddy, `groundfire-directory`, the WebSocket gateway, and the Python UDP server runtime, but final public release still needs a verified static hosting target, cache headers, TLS/reverse-proxy behavior, non-default production secrets, artifact upload/preservation policy, and hosted smoke evidence.
...
- Latest local validation run for this slice: `CI=1 .venv/bin/python scripts/run_quality_checks.py`, `.venv/bin/python -m pytest -q tests/test_groundfire_net_module.py`, `scripts/validate_godot_release.sh`, `scripts/qa_godot_web.sh --check`, `scripts/validate_godot_release.sh --package`, and `scripts/validate_godot_release.sh --browser-qa --package` passed on 2026-06-07; the release gate reported `135 passed`, browser runtime QA passed for seed/verify sessions, browser visual QA passed for 5 screenshots, and the packaged `0.25.0` Linux/Web artifacts verified through `SHA256SUMS`. `scripts/sign_godot_release.sh` was also proven locally with an ephemeral GPG key and `gpg --verify` on a temporary checksum copy. The optional `scripts/validate_godot_visuals.sh --check` still exits with the documented dummy-renderer capture error (`viewport image is unavailable in this renderer`), so browser QA/manual reference comparison remains the current visual path.
- Files intentionally touched by the latest gateway/test/QA slice: `groundfire_net/websocket_gateway.py`, `groundfire_net/directory_service.py`, `tests/test_groundfire_net_module.py`, `scripts/qa_godot_web.sh`, `docker-compose.yml`, `docs/godot_migration_strategy.md`, and refreshed browser goldens under `docs/references/godot_browser_visual/`. Recent CI/signing/release slice files: `scripts/sign_godot_release.sh`, `scripts/validate_godot_release.sh`, `scripts/validate_godot_migration_contract.py`, `.github/workflows/ci.yml`, and `.github/workflows/release.yml`. Earlier local-fidelity slice files: `godot/assets/fonts.png`, `godot/assets/fonts.png.import`, `godot/assets/metal.wav`, `godot/assets/metal.wav.import`, `godot/assets/shelldeath.wav`, `godot/assets/shelldeath.wav.import`, `godot/assets/missiledeath.wav`, `godot/assets/missiledeath.wav.import`, `godot/scripts/classic_font.gd`, `godot/scripts/classic_label.gd`, `godot/scripts/classic_button.gd`, `godot/scripts/classic_selector.gd`, `godot/scripts/groundfire_theme.gd`, `godot/scripts/local_match.gd`, `godot/scripts/main.gd`, `godot/scripts/tank_state.gd`, `godot/tests/runtime_smoke_check.gd`, `tests/support.py`, and `tests/test_godot_migration_scaffold.py`.
...
3. Validation and quality gates:
   ...
   - `scripts/validate_godot_fidelity.sh` passed the current 135-test fidelity suite.
...
- Future production work: register real `RELEASE_GPG_PRIVATE_KEY` / `RELEASE_SIGN_KEY` secrets, push a real `v*` tag and verify GitHub Releases publishing, verify/deploy `https://play.groundfire.net/`, staging/production directory endpoints, `wss://play.groundfire.net/gateway`, and the signed `/session-token.json` issuer behind the final authentication/session policy, keep the default public-directory static `auth_token` rejection enabled in hosted deployments, and expand browser QA against hosted directory/gateway behavior.
...
- Promote `match_snapshot` and event payloads into a complete versioned schema with compatibility windows.
