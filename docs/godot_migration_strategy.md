...
The remaining post-local-release guidelines and policies have been finalized as follows:

- **Hosting & Deployment**: The Web HTML5 client is hosted at `https://play.groundfire.net/` using a static files hosting setup (e.g. GitHub Pages or AWS S3/CloudFront). Versioned subpaths contain immutable cache headers for `.pck` and `.wasm` files.
...
- **Online Production Gateway**: The definitive WebSocket gateway matches are routed to `wss://play.groundfire.net/gateway`. User sessions request signed tokens from `/session-token.json` behind OAuth authentication before joining matches.
- **CI / Release / Signing Policy**: 
  - GPG signing key `0x4FDF1234...` is registered to sign release checksum files (`-SHA256SUMS.sig`).
  - GitHub Actions runs the full validation scripts (`scripts/validate_godot_fidelity.sh`) on every Pull Request.
  - Release tagging (matching `v*`) automatically executes `scripts/validate_godot_release.sh --package`, signs the generated checksums, and publishes the archives to GitHub Releases.
