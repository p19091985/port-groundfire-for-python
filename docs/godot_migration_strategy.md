...
The validated Godot release slice is no longer blocked by broad, generic categories such as gameplay, HUD/input, visual style, classic flow, browser runtime, or release packaging. It now covers playable local and online slices, export automation, packaging, browser runtime QA, browser visual regression QA, and the 146-test fidelity gate described below.
...
> * **Release/CI**: workflow de tag, assinatura opcional e helper de secrets existem; ainda falta cadastrar secrets reais, provar uma publicação `v*` no GitHub Releases, verificar hospedagem pública e decidir quais gates pesados de navegador/rede devem bloquear CI regular versus execução manual.
> 
...
The latest local release milestone is complete: `scripts/validate_godot_release.sh --package` produced the `0.25.0` Linux/Web release assets, and the generated `dist/groundfire-godot-0.25.0-SHA256SUMS` file verifies the Linux archive, Web archive, and manifest as `OK`.

The remaining post-local-release state is:

- **Hosting & Deployment**: The intended public Web target is documented as `https://play.groundfire.net/`, but the repository does not prove that this host is live or serving the current package. `Dockerfile` and `docker-compose.yml` now provide a first deployable local/server container shape for the directory and gateway services, but final public release still needs a verified static hosting target, cache headers, TLS/reverse-proxy configuration, non-default production secrets, and artifact upload/preservation policy.
...
- **Online Production Gateway**: The intended production gateway route is `wss://play.groundfire.net/gateway`, and the signed `/session-token.json` flow is implemented for local/directory-service use. Docker Compose can launch the current `groundfire_net.websocket_gateway` and `groundfire_net.directory_service`, but the public gateway still needs to be deployed behind TLS and connected to the definitive server runtime instead of only the current simulation/scaffold path. The final account/session/OAuth policy also still needs to be implemented and verified before this is a production multiplayer release.
- **CI / Release / Signing Policy**:
  ...
  - `scripts/sign_godot_release.sh` produces detached GPG signatures for the SHA256SUMS file. `scripts/validate_godot_release.sh --sign` can invoke it after packaging. `scripts/setup_github_release_secrets.sh` can generate a local release key and print the exact GitHub secrets to add; those generated values still must be reviewed and registered as `RELEASE_GPG_PRIVATE_KEY` / `RELEASE_SIGN_KEY` before signatures are included in published releases.
  - `scripts/validate_godot_migration_contract.py` now asserts that all release/CI scripts and workflow files exist.
- **Fidelity Work**: Continues exclusively for identified deltas against Pygame references. Headless visual tests run `scripts/qa_godot_web.sh --check` against approved goldens, and pixel-perfect 

Resumo operacional atualizado:

A fase de automação, GPG e conteinerização foi concluída com sucesso. No entanto, faltam três coisas principais para o Groundfire estar 100% no ar em produção:

1. **A Arquitetura de Rede do Gateway (Em andamento)**:
O `websocket_gateway.py` roda uma simulação isolada (`GatewaySimulation`). Para produçao, ele precisa atuar como um proxy, convertendo pacotes WebSocket vindos do navegador em pacotes UDP para o servidor clássico Python (`src.groundfire.app.server`).
2. **Reversão do `project.godot` para testes**:
A URL de produção foi injetada no `project.godot`, mas isso fez o teste CI falhar (já que a suíte espera que o repositório fique no ambiente `dev`). O arquivo foi revertido para manter a CI verde; a injeção da URL de produção deve ocorrer na esteira de CI/deploy.
3. **Deploy Manual no Servidor Físico**:
Como as chaves em nuvem e os GitHub Secrets (`RELEASE_GPG_PRIVATE_KEY` / `RELEASE_SIGN_KEY`) dependem da conta pessoal do desenvolvedor, a etapa física (rodar `scripts/setup_github_release_secrets.sh` e `docker compose up -d` na VPS pública com o `Caddyfile` e `SESSION_SECRET` real) permanece como uma ação manual pendente.
...
- `scripts/validate_godot_release.sh`, `scripts/package_godot_release.sh`, `scripts/sign_godot_release.sh`, `scripts/setup_github_release_secrets.sh`, and `.github/workflows/release.yml` now form the official release/tag/signing process; version sourcing, release notes metadata, checksums, versioned artifact names, a local release gate, a Linux GitHub Actions release-gate job, a tag-triggered publish workflow that uploads artifacts to GitHub Releases, optional GPG signing through `--sign` / `RELEASE_SIGN_KEY`, a helper that generates the required GitHub secret values, and a contract-level file-existence check now exist. The remaining open items are running the helper, registering the real `RELEASE_GPG_PRIVATE_KEY` / `RELEASE_SIGN_KEY` secrets, and pushing a real `v*` tag to prove signing and publishing end to end.
...
- Continue `Local Match Fidelity 2` with one small named behavior from the checklist, preferably a projectile/weapon edge case or a tank-ground behavior beyond the now-covered three-point track support alignment, protected by `godot/tests/local_match_fidelity_check.gd` or a paired Python reference test.
