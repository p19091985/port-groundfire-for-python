...
- Latest local validation run for this slice: `CI=1 .venv/bin/python scripts/run_quality_checks.py` passed on 2026-06-07 with `342 passed, 12 subtests passed`. The latest `test_port_fidelity.py` expansion added two new test classes: `ScoreEconomyFidelityTests` (8 tests covering every individual rule in `player.end_round()` — stipend, survival, non-leader defeat, leader defeat, self-defeat, accumulation across rounds, and combined expectation) and `AIPlayerShopFidelityTests` (3 tests verifying the classic AI shop cursor: GUNUP when not at Done!, FIRE when at position 10, and that position 0 is not Done!). All 342 tests pass. `scripts/run_quality_checks.py` (`compileall`, `unittest`, `ruff`, `mypy`) all pass.
