...
- Continue matching button width, height, typography, hover state, focus state, and disabled state; the main menu now uses the Pygame reference logo/version/copyright/button stack, classic brown/black surfaces, and a closer flush button stack inside the classic panel, but final font-atlas text rendering and exact hover/disabled parity remain.
...
- Continue expanding Options into final classic parity; richer video/audio/gameplay settings, classic-facing Resolution/Screen Mode preset rows, Set Controls jump, Apply/Back actions, pause access, and grouped Video/Audio/Gameplay/Online/Controls panels now exist, but exact selector arrow styling, font-atlas text rendering, and final layout polish remain.
...
- Continue visual parity for Main Menu and Options typography/hover/pixel review now that the classic Options preset rows and menu button stack are in place.
- Compare the Godot screens against `docs/references/pygame_visual/main_menu.png` and `options.png`, focusing on font-atlas-like text spacing/shadow, selector arrow styling, hover/focus state, and exact vertical placement.
