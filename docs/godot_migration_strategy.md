...
- Latest local validation run for this slice: `CI=1 .venv/bin/python scripts/run_quality_checks.py` passed on 2026-06-07 with `355 passed, 12 subtests passed`. The latest `test_port_fidelity.py` expansion added 5 missing tests for weapon logic (`WeaponFidelityTests`): `test_nuke_weapon_fire_consumes_inventory_and_spawns_whiteout_shell`, `test_mirv_splits_into_exact_fragment_count_at_apex`, `test_mirv_fragments_inherit_zero_vertical_velocity_and_spread_horizontally`, `test_mirv_ground_collision_before_apex_explodes_without_splitting`, and `test_mirv_tank_collision_before_apex_explodes_without_splitting`. This locks in the classic projectile inventory and apex-splitting behaviors before any Godot-side weapon tuning. All 355 tests pass. `scripts/run_quality_checks.py` (`compileall`, `unittest`, `ruff`, `mypy`) all pass.
