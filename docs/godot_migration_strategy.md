...
- **Hosting & Deployment**: The intended public Web target is documented as `https://play.groundfire.net/`, but the repository does not prove that this host is live or serving the current package. Final public release still needs a verified static hosting target, cache headers, and artifact upload/preservation policy.
...
- **Online Production Gateway**: The intended production gateway route is `wss://play.groundfire.net/gateway`, and the signed `/session-token.json` flow is implemented for local/directory-service use. A deployed gateway connected to the definitive server runtime, plus the final account/session/OAuth policy, still needs to be implemented and verified before this is a production multiplayer release.
- **CI / Release / Signing Policy**:
  ...
  - `scripts/sign_godot_release.sh` produces detached GPG signatures for the SHA256SUMS file. `scripts/validate_godot_release.sh --sign` can invoke it after packaging. The scripts are ready; a real signing key/fingerprint must still be registered as `RELEASE_GPG_PRIVATE_KEY` / `RELEASE_SIGN_KEY` repository secrets before signatures are included in published releases.
  - `scripts/validate_godot_migration_contract.py` now asserts that all release/CI scripts and workflow files exist.
- **Fidelity Work**: Continues exclusively for identified deltas against Pygame references. Headless visual tests run `scripts/qa_godot_web.sh --check` against approved goldens, and pixel-perfect comparison uses the authoritative assets under `docs/references/pygame_visual/`.
...
- `scripts/validate_godot_release.sh`, `scripts/package_godot_release.sh`, `scripts/sign_godot_release.sh`, and `.github/workflows/release.yml` now form the official release/tag process; version sourcing, release notes metadata, checksums, versioned artifact names, a local release gate, a Linux GitHub Actions release-gate job, a tag-triggered publish workflow that uploads artifacts to GitHub Releases, optional GPG signing through `--sign` / `RELEASE_SIGN_KEY`, and a contract-level file-existence check now exist. The remaining open item is choosing and registering a real GPG key (`RELEASE_GPG_PRIVATE_KEY` / `RELEASE_SIGN_KEY` secrets) to enable signed releases in CI.
