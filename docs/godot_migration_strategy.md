...
- Latest local validation run for this slice: `CI=1 .venv/bin/python scripts/run_quality_checks.py` passed on 2026-06-07 with `365 passed, 12 subtests passed`. The latest `test_port_fidelity.py` expansion added 6 missing tests for Shop Economy (`ShopMenuEconomyFidelityTests`): `test_shop_purchase_machine_gun_adds_50_ammo`, `test_shop_purchase_jump_jets_adds_one_total_fuel`, `test_shop_purchase_missiles_adds_5_ammo_and_nukes_mirvs_add_1`, `test_shop_purchase_prevents_buy_without_sufficient_funds`, `test_shop_input_delay_prevents_duplicate_buy_actions`, and `test_shop_done_position_marks_player_ready`. This locks in the classic weapon bundle sizes, input delays, and economy rules before any Godot-side tuning. All 365 tests pass. `scripts/run_quality_checks.py` (`compileall`, `unittest`, `ruff`, `mypy`) all pass.
